...
contract SimplexVerifierMultisig is SimplexVerifierBase {
    ...
    function _extractBitmapAndSignature(
        ...
        (bitmapLengthInBits, signersBitmap, endOffset) =
            CodecHelpers.deserializeSignersBitmap(proof, startOffset, maxParticipants);

        // Extract aggregated signature
        uint256 sigLen = SIGNATURE_LENGTH();
        if (endOffset + sigLen > proof.length) revert CodecHelpers.InvalidProofLength();
    ...
    /// @notice Verify a certificate with aggregated BLS signature (memory version)
    /// @dev Core verification logic that aggregates public keys and verifies the signature
    ...
    function _verifyCertificate(
        ...
        // Build signed message using union_unique format
        bytes memory signedMessage = encodeSignedMessage(namespaceWithSuffix, messageBytes);

        // Verify the aggregated signature using the scheme
        return scheme.verifySignature(signedMessage, aggregatedPublicKey, aggregatedSignature);
    }

    /// @notice Verify a certificate with aggregated BLS signature (calldata version for external keys)
    ...
    function verifyNotarizeWithIndex(
        ...
        bytes memory publicKey = participants[signer];
        bytes memory namespaceWithSuffix = abi.encodePacked(namespace, "_NOTARIZE");
        bytes memory signedMessage = encodeSignedMessage(namespaceWithSuffix, proposalBytes);
        return scheme.verifySignature(signedMessage, publicKey, signature);
    }
